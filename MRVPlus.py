...
# checks if b is a peer cell to a
def peers(a, b):
    row = a[0]
    column = a[1]
    if row == b[0] or column == b[1] or \
    (b[0] > row-row%3-1 and b[0] < row-row%3+3 and \
    b[1] > column-column%3-1 and b[1] < column-column%3+3):
        return True
    return False
